    ...
    public partial class MoviesPage : Form
    {
       ...
       (
        ...
        private async void LoadNewReleases()
        {
            ...
            string response = await service.GetAsync("/home/getnewmovies?isTVShows=false");
        ...
        private async void LoadTopScore()
        {
            ...
            HttpClientService service = new HttpClientService();
            string response = await service.GetAsync("/user/gettopmoviesbyimdbscorebutnotinnewmovies?isTVShows=false");
            if (response.StartsWith("Error"))
            {
                MessageBox.Show(response, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
